...
pub fn state(ctx: &ReadCtx, args: &[&str], out: &mut impl Write) -> Result<()> {
    ...
    writeln!(
        out,
        "  energy-ledger: org={:.6}->{:.6} food={:.6}->{:.6} artifact={:.6}->{:.6} plant_spawn={:.6} metabolism={:.6} action={:.6} protocol_cost={:.6} retention_loss={:.6} artifact_release_loss={:.6} removal_adjustment={:.6} food_split_residual={:.3e} artifact_release_residual={:.3e} transfer_residual={:.3e} total_residual={:.3e} tol={:.1e}",
        ledger.organism_energy_before,
        ledger.organism_energy_after,
        ledger.food_energy_before,
        ledger.food_energy_after,
        ledger.artifact_energy_before,
        ledger.artifact_energy_after,
