...
Read `CLAUDE.md` and `docs/cli.md` in full before acting. This substrate has
...
`world-sim`, with Hebbian plasticity as the only online learning rule). Evolution
operates over genomes; `world-sim` is a pure clonal evaluator and NEAT
mutation/crossover/selection lives in `world-sim/src/evolution.rs`. A run is
...
`cli` evidence — not merely proposed on paper.
...
- **Ground every claim in the tooling.** Use `cli` (stateless world-as-file:
  ...
  experiments, and `evolution` (release mode, multi-seed) for
...
- **Treat the metrics as raw, not as truth.** `pillars` emits raw windowed
  ...
  against §2.
- **Use the canonical world config.** `config/world.toml` is the one
  world/evolution environment loaded by cli, evolution, sim-server,
  and `WorldConfig::default`; do not introduce mirrored world TOMLs. Seed-genome
...
- **Metric-gaming**: does the headline metric move while behavior is static or
  ...
  (`cli find/top/inspect/brain/decide`), not just aggregate scores.
- **Seed robustness**: does it reproduce across the `evolution` seed suite,
...
changes, reproducible `cli`/`evolution` commands, metric trajectories,
