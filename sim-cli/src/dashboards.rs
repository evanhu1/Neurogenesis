//! Aggregate dashboard commands: ecological dynamics, lineage/species
//! composition, genome-gene drift, recorded timeseries, and a watch loop.
//!
//! These are `impl App` methods living in their own module; they may freely use
//! `App`'s fields/helpers (`self.sim`, `self.recorder`, `self.report_every`,
//! `self.take_format`, `self.live_pillars`, `crate::food_summary`) and the
//! shared output helpers in `crate::output`.
...
fn downsample(vals: &[f64]) -> Vec<f64> {
    ...
    for b in 0..SPARK_CAP {
        ...
        let hi = ((b + 1) * vals.len() / SPARK_CAP).max(lo + 1).min(vals.len());
...
impl App {
    pub(crate) fn eco(&mut self, args: &[&str], out: &mut impl Write) -> Result<()> {
        ...
        if let Some(rec) = recorder.filter(|_| have_traj) {
            ...
            let preds: Vec<f64> = s.iter().map(|x| x.predations as f64).collect();
            let tail = (s.len() - s.len() / 5).min(s.len() - 1);
            let cc = mean(&s[tail..].iter().map(|x| x.population as f64).collect::<Vec<_>>());
            ...
            writeln!(out, "  (no trajectory: `record on` then advance to populate)")?;
        }
        Ok(())
    }

    pub(crate) fn lineage(&mut self, args: &[&str], out: &mut impl Write) -> Result<()> {
        let (fmt, _) = self.take_format(args);
        let sim = self
            .sim
            .as_ref()
    ...
    pub(crate) fn genome(&mut self, args: &[&str], out: &mut impl Write) -> Result<()> {
        ...
        // (name, group, extractor). num_neurons is the genome gene; synapses use
        // the expressed brain count (genome.topology.num_synapses is the wiring
        // budget gene, but brain.synapse_count is the realized count).
        type Extractor = fn(&OrganismState) -> f64;
        let genes: &[(&str, &str, Extractor)] = &[
            ("num_neurons", "topology", |o| {
                o.genome.topology.num_neurons as f64
            }),
            ("num_synapses", "topology", |o| {
                o.brain.synapse_count as f64
            }),
        ...
        let mut_genes: &[(&str, MutExtractor)] = &[
            ("age_of_maturity", |o| o.genome.mutation_rates.age_of_maturity as f64),
            ("gestation_ticks", |o| o.genome.mutation_rates.gestation_ticks as f64),
            ("max_organism_age", |o| o.genome.mutation_rates.max_organism_age as f64),
            ("vision_distance", |o| o.genome.mutation_rates.vision_distance as f64),
            ("hebb_eta_gain", |o| o.genome.mutation_rates.hebb_eta_gain as f64),
            ("juvenile_eta_scale", |o| o.genome.mutation_rates.juvenile_eta_scale as f64),
            ("inter_bias", |o| o.genome.mutation_rates.inter_bias as f64),
            ("inter_update_rate", |o| o.genome.mutation_rates.inter_update_rate as f64),
            ("eligibility_retention", |o| o.genome.mutation_rates.eligibility_retention as f64),
            ("synapse_prune_threshold", |o| o.genome.mutation_rates.synapse_prune_threshold as f64),
            ("synapse_weight_perturbation", |o| o.genome.mutation_rates.synapse_weight_perturbation as f64),
            ("add_synapse", |o| o.genome.mutation_rates.add_synapse as f64),
            ("remove_synapse", |o| o.genome.mutation_rates.remove_synapse as f64),
            ("remove_neuron", |o| o.genome.mutation_rates.remove_neuron as f64),
            ("add_neuron_split_edge", |o| o.genome.mutation_rates.add_neuron_split_edge as f64),
            ("max_weight_delta_per_tick", |o| o.genome.mutation_rates.max_weight_delta_per_tick as f64),
        ];
        ...
        // recorder. The recorder does not retain per-gene history, so two
        // population samples aren't available — emit a clear note instead.
    ...
    pub(crate) fn timeseries(&mut self, args: &[&str], out: &mut impl Write) -> Result<()> {
        let (fmt, rest) = self.take_format(args);
        ...
        let current_turn = self
            ...
            .as_ref()
            .map(|s| s.turn())
            .ok_or_else(|| anyhow!("no simulation loaded"))?;

        // Sample-keyed columns (per recorded tick).
        let s = &rec.samples;
        let sample_col = |name: &str| -> Option<Vec<f64>> {
            let f: fn(&crate::EcoSample) -> f64 = match name {
        ...
        let intervals =
            derive_interval_metrics(&rec.tick_summary, &rec.lifetimes, report_every, current_turn);
    ...
    fn emit_watch_row(&self, fmt: crate::output::Format, out: &mut impl Write) -> Result<()> {
