...
use super::*;
use config::food_ecology_policy;
...
impl Simulation {
    ...
    fn regrowth_delay_turns(&self) -> u64 {
        u64::from(self.config.food_regrowth_interval)
    }
    ...
    fn spawn_food_at_cell(&mut self, cell_idx: usize) -> Option<FoodState> {
        let energy = self.config.food_energy;
        if self.occupancy[cell_idx].is_some() || energy == 0 {
            return None;
        }
        let width = self.config.world_width as usize;
        let q = (cell_idx % width) as i32;
        let r = (cell_idx / width) as i32;
        let visual = crate::jitter_visual_rgb_uniform(
            types::plant_visual(),
            &mut self.rng,
            crate::PLANT_COLOR_JITTER,
        );
        let food = FoodState {
